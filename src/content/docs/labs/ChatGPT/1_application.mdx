...
6. **Add the Tauri commands**
    ...
    - `async fn prompt(app: AppHandle, msg: String) -> Result<String, ()>`: Sends a message to the ChatGPT API. This will also set up an emitter for events carrying payloads from the API response, so we can listen to them in the FE.
    <ShowSolution>
    TODO: Develop this part
    ```sh frame="none"
    ```
    </ShowSolution>
7. **Connect the FE with the BE**
...
### How would you attack this solution?
