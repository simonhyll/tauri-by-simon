...
export const tauriArticles = (await getCollection('docs')).filter(val=>val.slug.includes('concepts/legal')).filter(val=>!val.slug.endsWith('/legal')).sort((a, b) => a.data.sidebar.order - b.data.sidebar.order);
