...
If you are going to use it on Windows, make sure to read the [WSL2](/setup/windows/wsl2) instructions on setting that up before you start using this.
...
```json title=".devcontainer/devcontainer.json"
{
    ...
    "build": {
        ...
        "args": {
          // All Dockerfile ARGs can be overridden here
        }
  },
  "customizations": {
    "settings": {
...
RUN <<EOT 
    ...
    apt install -yq --no-install-recommends ${TAURI_DEPENDENCIES} ${EXTRA_DEPENDENCIES}
EOT
